...
namespace MercuryBus.UnitTests.Consumer.Database
{
    public class SqlTableBasedDuplicateMessageDetectorTests
    {
        private SqlTableBasedDuplicateMessageDetector _detector;
        private MercuryBusDbContext _dbContext;

        [SetUp]
        public void SetupDetector()
        {
            var dbContextProvider = Substitute.For<IMercuryBusDbContextProvider>();
            _dbContext = Substitute.For<MercuryBusDbContext>();
        ...
        private static SqlException CreateSqlException(int number, string message)
        {
            ...
            var error = errorConstructor.Invoke(new object[]
                {number, (byte) 0, (byte) 0, "server", message, "procedure", 100, (uint) 0, null});
            ...
            return (SqlException) constructor.Invoke(new object[]
                {message, errorCollection, new DataException(), Guid.NewGuid()});
        }
    }
}
